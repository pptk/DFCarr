...
@property (nonatomic,strong)    DFAttributedLabelURL *touchedLink;
@property (nonatomic,assign)    BOOL linkDetected;
@property (nonatomic,assign)    BOOL ignoreRedraw;
@end

@implementation DFAttributedLabel
...
{
    [self setFont:font range:NSMakeRange(0, _attributedString.length)];
}
-(void)setFont:(UIFont *)font range:(NSRange)range{
    if(font && _font != font){
        ...
        [self resetFont];
        for(DFAttributedLabelAttachment *attachment in _attachments){
...
{
    [self setTextColor:textColor range:NSMakeRange(0, _attributedString.length)];
}
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (DFAttributedLabelURL *url in _linkLocations)
        {
...
- (DFAttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            DFAttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    DFAttributedLabelURL *url = [self urlForPoint:point];
...
- (DFAttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (DFAttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (DFAttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
             ...
             margin: (UIEdgeInsets)margin
{
    [self appendImage:image
              maxSize:maxSize
               margin:margin
            alignment:DFImageAlignmentBottom];
}
...
{
    DFAttributedLabelAttachment *attachment = [DFAttributedLabelAttachment attachmentWith:image
                                                                                     margin:margin
                                                                             alignment:alignment
                                                                               maxSize:maxSize];
...
- (void)appendView: (UIView *)view
         ...
         alignment: (DFImageAlignment)alignment
{
    DFAttributedLabelAttachment *attachment = [DFAttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
{
    DFAttributedLabelURL *url = [DFAttributedLabelURL urlWithLinkData:linkData
                                                                  range:range
                                                                  color:color];
...
{
    [DFAttributedLabelURL setCustomDetectMethod:block];
}
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            DFAttributedLabelAttachment* attributedImage = (DFAttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (BOOL)onLabelClick:(CGPoint)point
{
    ...
    if (linkData)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(dfAttributedLabel:clickedOnLink:)])
        {
            [_delegate dfAttributedLabel:self clickedOnLink:linkData];
        }
        else
        {
            NSURL *url = nil;
...
- (void)recomputeLinksIfNeeded
{
    ...
    BOOL sync = length <= DFMinAsyncDetectLinkLength;
               ...
               sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if ([links count])
        {
            for (DFAttributedLabelURL *link in links)
            {
    ...
    {
        _ignoreRedraw = YES;
        NSArray *links = [DFAttributedLabelURL detectLinks:text];
        block(links);
        _ignoreRedraw = NO;
    }
    else
...
- (void)addAutoDetectedLink: (DFAttributedLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    DFAttributedLabelURL *touchedLink = [self urlForPoint:point];
